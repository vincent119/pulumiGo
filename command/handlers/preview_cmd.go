...
func NewPreviewCommand() *PreviewCommand {
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if cmd.Flag("import-file") != nil && cmd.Flag("import-file").Changed {
                importFile, _ := cmd.Flags().GetString("import-file")
                cmdArgs = append(cmdArgs, "--import-file", importFile)
            }
            ...
            if cmd.Flag("save-plan") != nil && cmd.Flag("save-plan").Changed {
                savePlan, _ := cmd.Flags().GetString("save-plan")
                cmdArgs = append(cmdArgs, "--save-plan", savePlan)
            }
